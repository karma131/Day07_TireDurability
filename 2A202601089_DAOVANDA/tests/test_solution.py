...
class TestKnowledgeBaseAgent(unittest.TestCase):
    ...
    def test_prompt_requires_complete_policy_coverage(self):
        captured = []
        store = EmbeddingStore(collection_name="kb_prompt_test", embedding_fn=_mock_embed)
        store.add_documents(
            [
                Document(
                    id="room-policy",
                    content=(
                        "Mỗi nhóm được đặt tối đa 2 giờ và có thể đặt trước "
                        "tối đa 1 tuần."
                    ),
                    metadata={},
                )
        ...
        agent = KnowledgeBaseAgent(
            store=store,
            llm_fn=lambda prompt: captured.append(prompt) or "Câu trả lời.",
        )

        agent.answer("Một nhóm được đặt phòng bao lâu?")

        self.assertIn("<context>", captured[0])
        self.assertIn("<question>", captured[0])
        self.assertIn("danh sách kiểm kê", captured[0])
        self.assertIn("không được bỏ sót bất kỳ con số", captured[0])

    def test_answer_appends_most_relevant_source_paragraph(self):
        store = EmbeddingStore(collection_name="kb_evidence_test", embedding_fn=_mock_embed)
        store.add_documents(
            [
                Document(
                    id="room-policy",
                    content=(
                        "Thiết bị được trả tại quầy.\n\n"
                        "Mỗi nhóm được đặt phòng 2 giờ và có thể đặt trước 1 tuần."
                    ),
                    metadata={},
                )
            ]
        )
        agent = KnowledgeBaseAgent(store=store, llm_fn=lambda prompt: "Tối đa 2 giờ.")

        answer = agent.answer("Nhóm được đặt phòng trong bao lâu?")

        self.assertIn("Thông tin đối chiếu trực tiếp từ chính sách", answer)
        self.assertIn("đặt trước 1 tuần", answer)
        self.assertNotIn("Thiết bị được trả", answer)
