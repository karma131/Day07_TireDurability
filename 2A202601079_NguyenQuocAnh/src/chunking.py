...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        pieces = [piece for piece in current_text.split(separator) if piece.strip()]
        chunks: list[str] = []
        for piece in pieces:
...
class HeadingSectionChunker:
    ...
    This chunker keeps headings attached to their content and groups adjacent
    sections until the chunk size budget is reached.
    """
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        fixed_chunks = FixedSizeChunker(chunk_size=chunk_size, overlap=max(0, min(50, chunk_size // 10))).chunk(text)
